...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    boundary = raw.find(b"\r\n\r\n")
    if boundary == -1:
        error = "Invalid HTTP response: Header and body boundary not found"
    ...
    try:
        ...
        if not header_lines or not header_lines[0]:
            error = "Invalid HTTP response: Status line missing"
            return status_code, preview, error
        ...
        if len(status_parts) < 2:
            error = f"Invalid HTTP status line: '{status_line}'"
            return status_code, preview, error
            
        try:
            status_code = int(status_parts[1])
        except ValueError:
            error = f"Invalid HTTP status code format: '{status_parts[1]}'"
            return status_code, preview, error
