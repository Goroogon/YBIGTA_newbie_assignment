...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h3 = z2 @ W3 + b3
        z3 = tanh(h3 + h1)  # skip connection: H3 + H1
 
        h4 = z3 @ W4 + b4
        y_hat = sigmoid(h4)
        y_hat = y_hat.reshape(X.shape[0],)  # reshape to
        #################
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        # dh1, db1, dW1
         # 'gradient 계산하는 과정'을 참고하여 gradient 계산
        # z3 = tanh(h3 + h1) 이므로, (h3 + h1)에 대한 gradient를 먼저 구한 뒤
        # skip connection을 통해 h3와 h1 양쪽에 모두 전달한다.
        d_sum = dz3 * (1 - z3 ** 2)  # d(tanh)/d(h3+h1)
        ...
        # h1은 z1 = ReLU(h1) 경로와 skip connection(z3 = tanh(h3+h1)) 경로
        # 두 곳 모두에 영향을 주므로 두 gradient를 더해준다.
        dh1 = dz1 * relu_grad(h1) + d_sum
